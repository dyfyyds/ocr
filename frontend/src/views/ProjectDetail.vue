  ...
  <el-container style="min-height: 100vh">
    ...
    <el-container>
      ...
      <el-main v-loading="pageLoading">
        ...
        <!-- 合同版本列表 -->
        <el-card v-if="contracts.length > 0" shadow="never" style="margin-bottom: 16px">
          <template #header><span style="font-weight: 600">合同文件版本（{{ contracts.length }}）</span></template>
          <el-table :data="contracts" stripe border size="small">
            <el-table-column prop="version" label="版本" width="70">
              <template #default="{ row }">v{{ row.version || 1 }}</template>
            </el-table-column>
            <el-table-column prop="file_type" label="类型" width="70">
              <template #default="{ row }">
                <el-tag :type="row.file_type === 'pdf' ? 'warning' : 'success'" size="small">
                  {{ row.file_type?.toUpperCase() }}
                </el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="file_name" label="文件名" show-overflow-tooltip />
            <el-table-column label="识别引擎" width="100">
              <template #default="{ row }">
                <el-tag v-if="row.source === 'paddleocr'" type="warning" size="small">PaddleOCR</el-tag>
                <el-tag v-else-if="row.source === 'python-docx'" type="success" size="small">Docx</el-tag>
                <span v-else style="color:#999">-</span>
              </template>
            </el-table-column>
            <el-table-column label="项目名称" show-overflow-tooltip>
              <template #default="{ row }">
                <span :class="{ 'highlight-empty': !row.extracted?.project_name }">{{ row.extracted?.project_name || '未识别' }}</span>
              </template>
            </el-table-column>
            <el-table-column label="合同金额" width="120">
              <template #default="{ row }">
                <span :class="{ 'highlight-empty': !row.extracted?.contract_amount }">
                  {{ row.extracted?.contract_amount ? '¥' + Number(row.extracted.contract_amount).toLocaleString() : '未识别' }}
                </span>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="80">
              <template #default="{ row }">
                <el-button v-if="row.raw_text" type="primary" link size="small" @click="showRawText(row)">查看原文</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>

        <!-- 多版本 OCR 对比（仅 2+ PDF 版本时显示） -->
        <el-card v-if="pdfVersions.length >= 2" shadow="never" style="margin-bottom: 16px">
          <template #header>
            <div style="display: flex; justify-content: space-between; align-items: center">
              <span style="font-weight: 600">多版本 OCR 字段对比（{{ pdfVersions.length }} 个版本）</span>
              <el-button type="primary" size="small" :loading="analyzing" @click="handleAnalyzeVersions">
                🤖 LLM 智能分析
              </el-button>
            </div>
          </template>
          ...
          <!-- 版本对比表 -->
          <el-table :data="versionDiffs" stripe border size="small" :row-class-name="row => row.hasDiff ? 'diff-row' : ''">
            <el-table-column prop="field.label" label="字段" width="120" />
            <el-table-column v-for="(v, i) in pdfVersions" :key="v.id" :label="'v' + (v.version || (i+1)) + ' ' + (v.file_name || '')" min-width="160">
              <template #default="{ row }">
                <span :style="{ color: row.hasDiff ? '#e6a23c' : '#333', fontWeight: row.hasDiff ? '600' : '400' }">
                  {{ formatOcrField(row.field.key, row.values[i]) || '未识别' }}
                </span>
              </template>
            </el-table-column>
            <el-table-column label="状态" width="80" align="center">
              <template #default="{ row }">
                <el-tag v-if="row.hasDiff" type="warning" size="small">有差异</el-tag>
          ...
          <!-- LLM 分析结果 -->
          <div v-if="versionAnalysis" style="margin-top: 16px">
            <div style="font-weight: 600; margin-bottom: 8px">🤖 AI 分析报告</div>
            <pre class="analysis-text">{{ versionAnalysis }}</pre>
          </div>
        </el-card>

        <!-- 原文弹窗 -->
        <el-dialog v-model="rawTextVisible" :title="'查看原文 - ' + (rawTextContract?.file_name || '')" width="700px">
          <pre class="raw-text" style="max-height: 500px">{{ rawTextContract?.raw_text }}</pre>
        </el-dialog>
...
const versionAnalysis = ref(null)

// PDF 版本列表（从 contracts 中筛选）
const pdfVersions = computed(() => contracts.value.filter(c => c.file_type === 'pdf'))

// 多版本对比数据：每个字段在各版本中的值
const versionDiffs = computed(() => {
  const versions = pdfVersions.value
  if (versions.length < 2) return []
  return ocrFields.map(f => {
    const values = versions.map(v => v.extracted?.[f.key] || '')
    const allSame = values.every(v => v === values[0])
    return { field: f, values, hasDiff: !allSame }
  })
})
...
const rawTextContract = ref(null)
function showRawText(contract) {
  rawTextContract.value = contract
...
const fieldMap = {
  project_name: '项目名称', contract_amount: '合同金额', contract_no: '合同编号',
  sign_date: '签订日期', customer_name: '客户名称',
}
const diffTypeMap = {
...
async function loadContracts() {
  try {
    const res = await getContracts(projectId)
...
// 多版本 LLM 分析
async function handleAnalyzeVersions() {
  analyzing.value = true
  try {
    const res = await analyzeVersions(projectId)
    versionAnalysis.value = res.analysis
    ElMessage.success('版本分析完成')
  } finally {
